...
/** Short badge label per delivery method, shown on each raffle card so
      ...
      <div className="my-3">
        ...
        <section>
          ...
          {(raffles ?? []).length === 0 ? (
            ...
            <div className="incentive-grid">
              {(raffles ?? []).map((raffle) => (
                <RaffleCardView key={raffle.id} raffle={raffle} symbol={symbol} />
              ))}
            </div>
          )}
        </section>
...
function RaffleCardView({ raffle, symbol }: { raffle: Raffle; symbol: string }) {
    ...
    <article className={`incentive-card${drawn ? ' incentive-card--reached' : ''}`}>
      {raffle.image_url && (
        <div className="incentive-prize-media">
          <img src={raffle.image_url} alt={raffle.name} />
        </div>
      )}
      <div className="incentive-card-body">
        <div className="d-flex justify-content-between align-items-start gap-2">
        ...
        <div className="d-flex gap-2 flex-wrap">
          <span className="incentive-badge incentive-badge--free">
            {DELIVERY_LABELS[raffle.delivery_method]}
          </span>
          {raffle.quantity > 1 && (
            <span className="incentive-badge incentive-badge--free">
