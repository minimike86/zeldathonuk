 ...
 * event that has at least one donation page configured — that way the
...
export function DonationCards({ children }: { children?: React.ReactNode }) {
  ...
  const event =
    events?.find((e) => e.is_active && e.donation_pages.length > 0) ??
    // The events viewset orders by -start_time, so the first match is the
    // most recently-started event that still has pages attached.
    events?.find((e) => e.donation_pages.length > 0) ??
    null;
