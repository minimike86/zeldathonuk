...
def draw_lid_tilt(d: ImageDraw.ImageDraw, ox: int, oy: int, angle: int) -> None:
    ...
    bx = ox + 10
    if angle == 30:
        # Lid pulled back, top edge raised ~6px
        d.polygon(
            [
                (bx, oy + 28),       # front-bottom
                (bx + 36, oy + 28),  # back-bottom
                (bx + 36, oy + 22),  # back-top (hinge)
                (bx, oy + 18),       # front-top (raised)
            ],
            fill=WOOD,
            outline=WOOD_DARK,
        )
        ...
        # Lid further back, almost vertical
        d.polygon(
            [
                (bx, oy + 28),
                (bx + 36, oy + 28),
                (bx + 36, oy + 16),  # back-top
                (bx + 8, oy + 6),    # front-top tipped well back
            ],
            fill=WOOD,
            outline=WOOD_DARK,
        )
        d.line((bx + 8, oy + 6, bx + 36, oy + 16), fill=GOLD)
        # Hint of glow starting to peek out
        d.ellipse((bx + 12, oy + 32, bx + 34, oy + 38), fill=GLOW)


def draw_lid_open(d: ImageDraw.ImageDraw, ox: int, oy: int) -> None:
    bx = ox + 10
    # Fully open — lid lying back along the top, glow inside the box
    d.polygon(
        [
            (bx, oy + 28),
            (bx + 36, oy + 28),
    ...
    d.line((bx + 6, oy + 4, bx + 36, oy + 12), fill=GOLD)
    # Glow blob inside, brighter at the centre
    d.ellipse((bx + 8, oy + 28, bx + 38, oy + 42), fill=GLOW)
    d.ellipse((bx + 14, oy + 30, bx + 32, oy + 38), fill=GLOW_BRIGHT)
    # Sparkles
    for sx, sy in ((bx + 12, oy + 22), (bx + 30, oy + 18), (bx + 22, oy + 14)):
        d.rectangle((sx, sy, sx + 1, sy + 1), fill=GLOW_BRIGHT)
